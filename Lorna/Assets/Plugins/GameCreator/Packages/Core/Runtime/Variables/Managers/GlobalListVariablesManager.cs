...
{
	[AddComponentMenu("")]
	public class GlobalListVariablesManager : GameCreatorSingleton<GlobalListVariablesManager>, IGameSave
	{
        ...
        public void ReLoadLocalAsset(){
            this.Values.Clear();
            GlobalListVariables[] nameVariables = VariablesRepository.Get.Variables.ListVariables;
            foreach (GlobalListVariables entry in nameVariables)
            {
                if (entry == null) return;
                Instance.RequireInit(entry);   
            }
  		...
  		public GlobalListVariables GetGlobalListVariablesAsset(string assetName)
  		{
        
            foreach (GlobalListVariables entry in VariablesRepository.Get.Variables.ListVariables)
            {

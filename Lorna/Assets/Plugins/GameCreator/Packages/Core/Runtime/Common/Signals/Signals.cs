...
namespace GameCreator.Runtime.Common
{
    ...
    {
        [NonSerialized] private static Dictionary<PropertyName, List<ISignalReceiver>> SIGNALS = new();
        ...
        //  自定义拓展，带参数的信号
        public static void Emit(SignalArgs args, string isString, bool bWithOutString = false)
        {
            if (ApplicationManager.IsExiting) return;

            Debug.Log("Signals Emit " + args.signal.ToString() + " with string " + isString);
            if (!SIGNALS.TryGetValue(args.signal, out List<ISignalReceiver> receivers))
            {
                return;
            }

            // Debug.Log("Signals Emit Enter receiver"+ args.signal.ToString()+" with string "+isString);
            foreach (ISignalReceiver receiver in receivers)
            {
                receiver.OnReceiveSignalString(args, isString);
                if (bWithOutString)
                    receiver.OnReceiveSignal(args);
            }
        }

...
//            double a = attribute.Value;
//            attribute.Value = (float)this.m_Change.Get(attribute.Value, args);
//            float ValueChange = (float)(attribute.Value - a);

//            if (ValueChange == 0)
//                return DefaultResult;

//            if (m_Attribute.ID.String != "HP")
//                return DefaultResult;

//            GameObject selfGo = args.Self;
//            var skillData = selfGo.GetComponentInParent<SkillData>();
//            if (skillData == null)
//            {
//                return DefaultResult;
//            }
//            StatsChangeArgs changeArgs = new();
//            foreach (var nv in args.variables)
//            {
//                if (nv.Name == "HittedCollider")
//                {
//                    changeArgs.HittedCollider = ((GameObject)nv.Value).GetComponent<Collider>();
//                    break;
//                }
//            }

//            changeArgs.DmNumber      = ValueChange;
//            changeArgs.AtkStrikeType = skillData.attackStrikeType;
//            skillData.SendMessage("CheckIsPlayer", changeArgs);


//            if (changeArgs.HitterIsPlayer)
//            {
//                //TODO:这里改成触发伤害跳字事件
//                Debug.Log(string.Format("伤害跳字：{0}，攻击类型：{1},坐标：{2}", ValueChange, skillData.attackStrikeType, changeArgs.position));
//            }
...
//            //Debug.Log(showType+":"+ValueChange);
//            //Debug.Log($"{target.name}-{this.m_Attribute.ID}: {a}->{attribute.Value}");
//            return DefaultResult;
//        }
//    }
//}
...
namespace NodeCanvas.DialogueTrees.UI.Examples
{
    ...
    public class DialogueUGUI : MonoBehaviour, IPointerClickHandler
    {
        ...
        void OnDialogueStarted(DialogueTree dlg) {
            ...
            if(AsideOffset == true){
                AsideOffset                                     =  false;
                subtitlesGroup.anchoredPosition                 += new Vector2(0, Screen.height - 70);
                actorSpeechImage.rectTransform.anchoredPosition -= new Vector2(0, 100);
            }
        ...
        void OnSubtitlesRequest(SubtitlesRequestInfo info) {
            if(info.statement.image == null){
                actorSpeechImage.gameObject.SetActive(false);
                actorSpeech.gameObject.SetActive(true);
                StartCoroutine(Internal_OnSubtitlesRequestInfo(info));
             }   
             else{
                    subtitlesGroup.gameObject.SetActive(true);
                    actorSpeech.gameObject.SetActive(false);
                    actorSpeechImage.gameObject.SetActive(true);
                    
                    var audio = info.statement.audio;
                    var actor = info.actor;
                    var sprite = info.statement.image;
                    actorName.text = actor.name;
                    actorSpeech.color = actor.dialogueColor;
                    actorPortrait.sprite = actor.portraitSprite;
                    actorSpeechImage.sprite = sprite;
                    actorSpeechImage.SetNativeSize();
             }
        }
          ...
          //subtitlesGroup.position = originalSubsPosition;
            ...
            if ( audio == null ) {
                ...
                for ( int i = 0; i < text.Length; i++ ) {
                    ...
                    if(c != '<')
                        tempText += c;
                    else //添加富文本文件 "<xxx>AAA</xxx>";
                    {
                        bool atFirstEnd = false; //第一个  '>';
                        bool atFinishtEnd = false; //第一个  '>';
                        do
                        {
                             tempText += c;
